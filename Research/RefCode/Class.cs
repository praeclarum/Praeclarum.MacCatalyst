...
namespace ObjCRuntime
{
	public class Class : INativeObject, IEquatable<Class>
	{
		...
		public string Name => Marshal.PtrToStringAuto(class_getName(handle));
		...
		private unsafe static bool CompareTokenReference(string asm_name, int mod_token, int type_token, uint token_reference)
		{
			...
			else
			{
				...
				uint num4 = (token_reference >> 1) & 0x7F;
		...
		private unsafe static int FindMapIndex(Runtime.MTClassMap* array, int lo, int hi, IntPtr @class)
		{
			...
			{
				int num = lo + (hi - lo) / 2;
				IntPtr value = array[num].handle;
				if (value == @class)
				{
					return num;
				}
				if (value.ToInt64() > @class.ToInt64())
				{
		...
		internal unsafe static Type FindType(IntPtr @class, out bool is_custom_type)
		{
			...
			type = ResolveTypeTokenReference(registrationMap->map[num].type_reference);
		...
		internal unsafe static MemberInfo ResolveFullTokenReference(uint token_reference)
		{
			...
			return ResolveToken(ResolveModule(ResolveAssembly(assembly_name), token), token2);
		}
		...
		private unsafe static MemberInfo ResolveTokenReference(uint token_reference, uint implicit_token_type)
		{
			...
			uint num = (token_reference >> 1) & 0x7F;
			uint num2 = (token_reference >> 8) + implicit_token_type;
			return ResolveToken(ResolveModule(ResolveAssembly(Marshal.ReadIntPtr(registrationMap->assembly, (int)num * IntPtr.Size)), 1u), num2 | implicit_token_type);
		}

		private static MemberInfo ResolveToken(Module module, uint token)
		{
			uint num = (uint)((int)token & -16777216);
			switch ((int)token & -16777216)
			{
			case 33554432:
				return module.ResolveType((int)token);
			case 100663296:
				return module.ResolveMethod((int)token);
			default:
				throw ErrorHelper.CreateError(8021, $"Unknown implicit token type: 0x{num:X}.");
			}
		}
		...
		internal unsafe static uint GetTokenReference(Type type, bool throw_exception = true)
		{
			...
			for (int i = 0; i < registrationMap->assembly_count; i++)
			{
				if (Runtime.StringEquals(Marshal.ReadIntPtr(registrationMap->assembly, i * IntPtr.Size), name))
				{
					num = i;
					break;
				}
		...
		private unsafe static uint GetFullTokenReference(string assembly_name, int module_token, int metadata_token)
		{
			...
			for (int i = 0; i < registrationMap->full_token_reference_count; i++)
			{
				...
				if (Marshal.ReadInt32(intPtr + IntPtr.Size + 4) == metadata_token && Marshal.ReadInt32(intPtr + IntPtr.Size) == module_token && Runtime.StringEquals(utf, assembly_name))
				{
					return (uint)((i << 1) + 1);
				}
			}
		...
		{
			if (GetClassHandle(type, throw_if_failure: false, out bool is_custom_type) != IntPtr.Zero)
			{
